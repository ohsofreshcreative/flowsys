	...
	<div class="c-main py-20">
		...
		<div class="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-8 lg:gap-12">
			<div>
				...
				{{-- SEKCJA DOFINANSOWAŃ - ZMIANY PONIŻEJ --}}
				@if($subsidies)
				<h4 class="text-2xl font-bold mb-4">Dofinansowania</h4>
				<div class="flex flex-col gap-2 mb-8">
					@foreach($subsidies as $subsidy)
					<label class="flex items-center gap-2">
						{{-- 1. Dodano atrybut data-amount z kwotą z ACF --}}
						<input
							type="checkbox"
							name="subsidies[]"
							value="{{ $subsidy['subsidy_name'] }}"
							data-amount="{{ $subsidy['subsidy_amount'] ?? 0 }}">
						{{-- 2. Wyświetlono kwotę obok nazwy --}}
						<span>{{ $subsidy['subsidy_name'] }} (do {{ number_format($subsidy['subsidy_amount'], 0, ',', ' ') }} zł)</span>
					</label>
					@endforeach
				</div>
				@endif
			...
			<div class="bg-white p-8 rounded-2xl h-fit sticky top-26">
				...
				<div class="mt-6">
					<button id="trigger-cf7-submit" class="w-full text-white bg-primary hover:bg-primary-dark focus:ring-4 focus:outline-none focus:ring-primary-light font-medium rounded-lg text-sm px-5 py-2.5 text-center">Wyślij zapytanie</sbutton>
				</div>
			</div>
		</div>
	</div>
