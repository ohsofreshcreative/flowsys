...
<section data-gsap-anim="section" @if(!empty($section_id)) id="{{ $section_id }}" @endif class="b-offer relative -spt {{ $sectionClass }} {{ $section_class }}">
	<div class="__wrapper c-main relative">
		...
		<div class="relative">
			...
			<img class="absolute -top-20 -left-40 pointer-events-none" src="/wp-content/uploads/2025/12/offer_stroke.svg" />
		</div>
		@endif

		<div class="__col grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10 mt-10">

			<div class="relative md:sticky top-10 h-max">
				@if (!empty($g_offer['image']))
				<div class="__img order1">
					<img class="absolute -bottom-10 -left-10 pointer-events-none" src="/wp-content/uploads/2025/12/sign_small.svg" />
					<img class="__img object-cover radius-img w-full img-xl" src="{{ $g_offer['image']['url'] }}" alt="{{ $g_offer['image']['alt'] ?? '' }}">
				</div>
				@endif
			</div>
				...
				<div class="__list mt-8">
					...
					<a href="{{ get_permalink($sector->ID) }}" class="">
						<div class="__card b-bottom-p-light flex items-center gap-6 mb-4 pb-4">
							...
							<div class="bg-p-light ml-auto rounded-full p-3 transition-all">
								<img class="" src="http://flowsys.local/wp-content/uploads/2025/12/arrow-right.svg" />
							</div>
						</div>
					</a>
					@endforeach
